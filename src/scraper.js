...
const DEFAULT_SECTIONS = ["General information", "VAT information", "Right of representation", "Contacts", "Shareholders", "Tax information", "Activity licenses and notices of economic activities", "Annual reports", "Areas of activity", "Articles of association", "Beneficial owners", "Data protection officer",];

function resolveJurisdictionEndpoints(code) {
    const normalized = (code || DEFAULT_JURISDICTION).toLowerCase();
    return JURISDICTION_ENDPOINTS[normalized] || JURISDICTION_ENDPOINTS.ee;
}

function getConfig(jurisdictionCode = DEFAULT_JURISDICTION) {
    const rawSections = process.env.WANTED_SECTIONS;
    const wantedSections = rawSections && rawSections.trim() ? rawSections
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean) : DEFAULT_SECTIONS;
    
    const endpoints = resolveJurisdictionEndpoints(jurisdictionCode);
    console.log("[getConfig] endpoints:", endpoints);
    const baseUrl =  endpoints.baseUrl || process.env.BASE_URL;
    const searchUrl = endpoints.searchUrl || process.env.SEARCH_URL;
    ...
    const config = {
        ...
        selectors: {
            ...
            autocompleteDropdown: process.env.SELECTOR_AUTOCOMPLETE_DROPDOWN || ".typeahead[role='listbox']",
            autocompleteItem: process.env.SELECTOR_AUTOCOMPLETE_ITEM || ".typeahead [role='option']",
        },
...
async function launchPage(jurisdictionCode = DEFAULT_JURISDICTION) {
    ...
    console.log("[launchPage] Launching browser", "jurisdiction", jurisdictionCode, "searchUrl", config.searchUrl,);
...
async function runSearch(page, config, query) {
    ...
    console.log("[runSearch] Waiting for search input selector:", config.selectors.searchInput,);
    ...
    console.log("[runSearch] Clicking search button:", config.selectors.searchButton,);
...
async function getCompanyByNameOrNumber(query, jurisdictionCode = DEFAULT_JURISDICTION,) {
    const normalizedJurisdiction = (jurisdictionCode || DEFAULT_JURISDICTION).toLowerCase();
    
    console.log("[getCompanyByNameOrNumber] Starting search for:", query, "jurisdiction", normalizedJurisdiction,);
    ...
    try {
        ...
        console.log("[getCompanyByNameOrNumber] Extracted", results.length, "results", normalizedJurisdiction,);
        
        if (results.length > 0) {
            const folderPath = getOutputFolder(normalizedJurisdiction);
            const safeName = `search-${sanitizeFilename(query)}`;
            await page.screenshot({
                path: path.join(folderPath, `${safeName}.jpg`), fullPage: true,
            });
            try {
                saveJsonFile(path.join(folderPath, `${safeName}.json`), results);
            } catch (err) {
                console.error("[saveJsonFile] Failed to save JSON:", err);
                fs.writeFileSync(path.join(folderPath, `${safeName}.json`), JSON.stringify({
        ...
        await browser.close();
        console.log("[getCompanyByNameOrNumber] Browser closed for jurisdiction", normalizedJurisdiction,);
    }
}
...
async function scrapeByUrl(url, jurisdictionCode = DEFAULT_JURISDICTION) {
    const normalizedJurisdiction = (jurisdictionCode || DEFAULT_JURISDICTION).toLowerCase();
    console.log("[scrapeByUrl] Starting scrape for URL:", url, "jurisdiction", normalizedJurisdiction,);
    const {browser, page, config} = await launchPage(normalizedJurisdiction);
    ...
    try {
        ...
        const officers = await page
            .$$eval("#representativesTable tbody tr", (rows) => rows.map((row) => {
        ...
        const ultimate_beneficial_owners = await page
            .$$eval("#beneficiaries-table tbody tr", (rows) => rows.map((row) => {
                const cells = [...row.querySelectorAll("td")].map((td) => td.textContent?.trim().replace(/\s+/g, " ") ?? "",);
                return {
                    name: cells[0], position: null, entityType: null, type_of_control: cells[2],
                };
            }),)
            .catch(() => []);
        ...
        await browser.close();
        console.log("[scrapeByUrl] Browser closed for jurisdiction", normalizedJurisdiction,);
    }
}
...
async function getAutocompleteSuggestions(query, jurisdictionCode = DEFAULT_JURISDICTION,) {
    const normalizedJurisdiction = (jurisdictionCode || DEFAULT_JURISDICTION).toLowerCase();
    console.log("[getAutocompleteSuggestions] Starting autocomplete for query:", query, "jurisdiction", normalizedJurisdiction,);
    const {browser, page, config} = await launchPage(normalizedJurisdiction);
    ...
    try {
        ...
        try {
            ...
            console.log("[getAutocompleteSuggestions] No dropdown appeared for query:", query,);
            return [];
        }
        
        console.log("[getAutocompleteSuggestions] Extracting suggestions...");
        const suggestions = await page
            .$$eval(config.selectors.autocompleteItem, (items) => items
                .map((item) => {
        ...
        await page.screenshot({
            path: path.join(folderPath, `${safeName}.jpg`), fullPage: false,
        });
        try {
            ...
            console.error("[saveJsonFile] Failed to save JSON:", err);
            fs.writeFileSync(path.join(folderPath, `${safeName}.json`), JSON.stringify({
                error: "Failed to serialize suggestions",
                details: String(err)
            }, null, 2,), "utf8",);
        }
        console.log(`[saved] ${folderPath}/${safeName}.{jpg,json}`);
        ...
        await browser.close();
        console.log("[getAutocompleteSuggestions] Browser closed for jurisdiction", normalizedJurisdiction,);
    }
}
